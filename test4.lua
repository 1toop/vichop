...
local cfgdata = {}
local cfgfile = "uicfg.json"

local function savecfg()
    pcall(function() writefile(cfgfile, http:JSONEncode(cfgdata)) end)
end

local function loadcfg()
    pcall(function()
        if isfile(cfgfile) then cfgdata = http:JSONDecode(readfile(cfgfile)) end
    end)
end
...
local function rgbtohsv(r, gr, b)
    return Color3.new(r / 255, gr / 255, b / 255):ToHSV()
end
                    ...
                    if Entered and Resizer.Active and options.can_resize then
                        while Held and Resizer.Active do
                            ...
                            elseif y >= options.min_size.Y then
                                ...
                                Resize(Window, { Size = UDim2.new(0, options.min_size.X, 0, options.min_size.Y) },
                                    options.tween_time)
                            end
        ...
        do -- Add Tab
            function window_data:AddTab(tab_name)
                ...
                do                          -- Tab Elements
                    ...
                    function tab_data:AddSwitch(switch_text, callback, switch_options) -- [Switch]
                        local switch_data = {}

                        switch_text = tostring(switch_text or "New Switch")
                        callback = typeof(callback) == "function" and callback or function() end
                        switch_options = typeof(switch_options) == "table" and switch_options or {}
                        ...
                        switch.MouseButton1Click:Connect(function()
                            ...
                            if switch_options.cfg then
                                cfgdata[switch_options.cfg] = toggled
                                savecfg()
                            end
                        ...
                        if switch_options.cfg and cfgdata[switch_options.cfg] ~= nil then
                            switch_data:Set(cfgdata[switch_options.cfg])
                        end
                    ...
                    function tab_data:AddSlider(slider_text, callback, slider_options)
                        ...
                        do -- Slider Math
                                    ...
                                    task.spawn(function() -- Loop check
                                        if Entered and not slider_options.readonly then
                                            while Held and (not dropdown_open) do
                                                local mouse_location = gMouse()
                                                local x = (slider.AbsoluteSize.X - (slider.AbsoluteSize.X - ((mouse_location.X - slider.AbsolutePosition.X)) + 1)) /
                                                ...
                                                Resize(indicator, { Size = UDim2.new(size or min, 0, 0, 20) },
                                                    options.tween_time)
                            ...
                            function slider_data:Set(new_value)
                                ...
                                value.Text = tostring(sel_value)
                                if slider_options.cfg then
                                    cfgdata[slider_options.cfg] = sel_value
                                    savecfg()
                                end
                                pcall(callback, sel_value)
                            end
                            
                            if slider_options.cfg and cfgdata[slider_options.cfg] then
                                local saved = cfgdata[slider_options.cfg]
                                local percent = ((saved - slider_options.min) / (slider_options.max - slider_options.min)) * 100
                                slider_data:Set(percent)
                            else
                                        ...
                                        while Held and Entered1 and (not dropdown_open) do -- Palette
                                            ...
                                            Resize(palette_indicator,
                                                { Position = UDim2.new(0,
                                                    math.abs(x - 100) - (palette_indicator.AbsoluteSize.X / 2), 0,
                    ...
                    function tab_data:AddConsole(console_options)
                        ...
                        do -- Highlighting (thanks to whoever made this)
                            local lua_keywords = { "and", "break", "do", "else", "elseif", "end", "false", "for",
                                "function", "goto", "if", "in", "local", "nil", "not", "or", "repeat", "return", "then",
                                "true", "until", "while" }
                            local global_env = { "getrawmetatable", "newcclosure", "islclosure", "setclipboard", "game",
                                "workspace", "script", "math", "string", "table", "print", "wait", "BrickColor", "Color3",
                                "next", "pairs", "ipairs", "select", "unpack", "Instance", "Vector2", "Vector3", "CFrame",
                                "Ray", "UDim2", "Enum", "assert", "error", "warn", "tick", "loadstring", "_G", "shared",
                                "getfenv", "setfenv", "newproxy", "setmetatable", "getmetatable", "os", "debug", "pcall",
                                "ypcall", "xpcall", "rawequal", "rawset", "rawget", "tonumber", "tostring", "type",
                                "typeof", "_VERSION", "coroutine", "delay", "require", "spawn", "LoadLibrary", "settings",
                                "stats", "time", "UserSettings", "version", "Axes", "ColorSequence", "Faces",
                                "ColorSequenceKeypoint", "NumberRange", "NumberSequence", "NumberSequenceKeypoint",
                                "gcinfo", "elapsedTime", "collectgarbage", "PhysicalProperties", "Rect", "Region3",
                                "Region3int16", "UDim", "Vector2int16", "Vector3int16", "load", "fire", "Fire" }
