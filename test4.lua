...
local function savecfg()
    pcall(function()
        if writefile then
            writefile(cfgfile, http:JSONEncode(cfgdata))
        end
    end)
end
...
local function loadcfg()
    pcall(function()
        if readfile and isfile and isfile(cfgfile) then
            cfgdata = http:JSONDecode(readfile(cfgfile))
        end
    end)
end
...
do
    for _, g in ipairs(pgui:GetChildren()) do
        if g:IsA("ScreenGui") and g:FindFirstChild("Prefabs") and g:FindFirstChild("Windows") then
            g:Destroy()
        end
    end
end
